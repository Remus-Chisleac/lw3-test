<div class="max-w-4xl mx-auto mt-8 p-6 bg-white rounded-lg shadow-lg">
    ...
    <h2 class="text-2xl font-bold mb-4 text-red-600">WITH wire:ignore.self (NEEDS REFRESH TRIGGER)</h2>
        ...
        <div class="flex gap-2 flex-wrap">
            @foreach($users as $user)
                <button 
                    ...
                    class="px-4 py-2 rounded {{ $selectedUser == $user['id'] ? 'bg-red-500 text-white' : 'bg-gray-200 hover:bg-gray-300' }}"
                >
                    {{ $user['name'] }}
                </button>
            @endforeach
    ...
    <div class="mt-6 p-4 bg-orange-50 border border-orange-200 rounded-md">
        <h4 class="font-semibold text-orange-800">What Happens:</h4>
        <ol class="text-sm text-orange-700 mt-2 space-y-1">
...
&lt;/div&gt;</code></pre>
            <p class="text-sm text-red-600 mt-2 font-medium">NEEDS TRIGGER: wire:key uses $selectedUser to force refresh since wire:ignore.self prevents updates</p>
        </div>
    </div>
</div>